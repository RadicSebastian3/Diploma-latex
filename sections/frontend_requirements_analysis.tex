...
Die funktionalen Anforderungen an das Frontend des \textit{SmartBillConverter}-Projekts definieren die Kernfunktionen des Systems:
...
Diese Anforderungen definieren die Qualität und Benutzerfreundlichkeit der \textit{SmartBillConverter}-Anwendung:
...
Das Entity-Relationship-Diagramm modelliert die zentrale Datenstruktur des \textit{SmartBillConverter}-Systems. Die \texttt{Invoice}-Entität repräsentiert im entwickelten Projekt eine verarbeitete Rechnungsdatei mit allen extrahierten Kerninformationen:

\begin{figure}[htbp]
\centering
\includegraphics[width=0.5\textwidth]{pics/erd.png}
\caption{Eigene Darstellung: Entity-Relationship-Diagramm der Invoice-Entität}
\label{fig:erd_diagram}
\end{figure}

\subsection{Detaillierte Attributbeschreibung}
Das Datenmodell des \textit{SmartBillConverter}-Projekts folgt den gesetzlichen Anforderungen des österreichischen UStG. Die einzelnen Attribute der im Projekt implementierten \texttt{Invoice}-Entität (siehe Abbildung \ref{fig:erd_diagram}) haben folgende Bedeutung:
...
Im \textit{SmartBillConverter}-Frontend wird ein TypeScript-Interface verwendet, das die Backend-Entität des Projekts widerspiegelt:
...
Use-Case-Diagramme visualisieren die Interaktionen zwischen Benutzern und dem System. Abbildung \ref{fig:use_case_diagram} zeigt die Hauptanwendungsfälle des \textit{SmartBillConverter}-Projekts:

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{pics/use-case-diagram.png}
\caption{Eigene Darstellung: Use-Case-Diagramm des SmartBillConverter}
\label{fig:use_case_diagram}
\end{figure}

\subsection{Beschreibung der Use-Cases}
Die in Abbildung \ref{fig:use_case_diagram} dargestellten Hauptanwendungsfälle des \textit{SmartBillConverter}-Systems werden im Folgenden detailliert beschrieben:

\textbf{UC1: Import Invoice} -- Der primäre Use-Case (siehe Abbildung \ref{fig:use_case_diagram}) ermöglicht das Hochladen von Rechnungsdokumenten. Der Benutzer wählt zwischen zwei Importvarianten:
