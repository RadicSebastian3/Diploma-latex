...
Die Prüfung schaut auf drei Dinge: komplett leer, nur Fehlermeldungen, oder nach dem Herausrechnen der Seitentrennzeilen weniger als 50 Zeichen. Trifft eines davon zu, wird in der aktuellen Projektversion ein kurzer Platzhaltertext angehängt. Dieser Platzhalter dient ausschließlich dazu, die nachfolgenden Verarbeitungsschritte der Pipeline (Normalisierung und XML-Erzeugung) im Test reproduzierbar ausführen zu können.

In einer Produktivumgebung ist so ein Platzhaltertext nicht sinnvoll, weil er eine leere oder unlesbare Rechnung verschleiert. Sinnvoller wäre hier eine klare Rückmeldung an den Benutzer (z.B. "Kein lesbarer Text erkannt") und anschließend ein automatischer OCR-Fallback (siehe nächster Abschnitt) oder ein Abbruch mit Hinweis auf eine bessere Eingabedatei.
