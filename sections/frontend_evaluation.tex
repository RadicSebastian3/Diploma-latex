...
Die erste Fehlerquelle ist das Hochladen falscher Dateiformate. Das System akzeptiert nur PDFs und gängige Bildformate (PNG, JPEG, BMP, TIFF, GIF). Wird ein anderes Format ausgewählt, blockiert das Frontend den Upload sofort und zeigt eine klare Fehlermeldung.
...
Wenn das Backend einen Fehler meldet (z.B. weil die Rechnung nicht lesbar ist oder wichtige Daten fehlen), wird die Fehlermeldung im Frontend angezeigt. Dadurch sieht der Benutzer nicht nur, dass etwas fehlgeschlagen ist, sondern auch warum.
...
Dieser Workflow ist absichtlich kurz gehalten, damit er ohne lange Einschulung funktioniert.

\textbf{Usability-Check:} Es wurde ein kurzer, aufgabenbasierter Test durchgeführt. Insgesamt nahmen \textbf{[ZAHL EINTRAGEN]} Personen teil (interne Kurztests). Die Aufgaben waren: Format wählen, Dateien hochladen, Konvertierung starten, Ergebnis herunterladen und eine fehlerhafte Datei wieder entfernen. Das Vorgehen und die Beobachtungskriterien sind im Anhang zusammengefasst (siehe Abschnitt \ref{app:usability_test}).

Als Ergebnis zeigte sich: Der Ablauf wurde in der Regel ohne Hilfe verstanden. Unklarheiten traten vor allem dann auf, wenn das Backend eine Fehlermeldung lieferte, die zwar korrekt war, aber ohne Kontext schwer einzuordnen ist (z.B. bei unlesbaren Scans). Hier hilft eine kurze Erklärung in der UI, was der Benutzer als nächstes tun kann.

\subsection{Testrechnungen: Bandbreite}
Für die Abnahme wurden zehn Testrechnungen verwendet. Die Originaldokumente sind aus Datenschutzgründen nicht im Text enthalten. Um dennoch nachvollziehbar zu machen, welche Bandbreite getestet wurde, werden die zehn Rechnungen im Anhang anonymisiert als TR01--TR10 charakterisiert (siehe Tabelle \ref{tab:testrechnungen}).

In Summe reichten die Testfälle von einfachen, klar lesbaren Ein-Seiten-PDFs bis zu schwierigeren Eingaben (z.B. mehrseitige Dokumente oder Scans mit sehr wenig extrahierbarem Text). Dadurch wurde sowohl der Normalfall als auch typische Problemfälle der Praxis abgedeckt.
