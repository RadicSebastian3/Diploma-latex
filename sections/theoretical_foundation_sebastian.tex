...
Die Gültigkeit einer ebInterface-Rechnung wird durch ein XML Schema (XSD) definiert. Pflichtfelder sind die Angaben, die das Umsatzsteuergesetz (UStG) für eine ordnungsgemäße Rechnung vorschreibt.\footnote{Vgl. § 11 Umsatzsteuergesetz 1994 (UStG 1994): \textit{Pflichtangaben einer Rechnung}, Rechtsinformationssystem des Bundes (RIS), \url{https://www.ris.bka.gv.at/GeltendeFassung.wxe?Abfrage=Bundesnormen&Gesetzesnummer=10004873}, letzter Zugriff am 07.03.2026} In Österreich gehören dazu unter anderem:
\begin{itemize}
...
\end{itemize}
Diese Architekturentscheidung löst ein in der Praxis häufig auftretendes Akzeptanzproblem: Empfänger ohne automatisierte Verarbeitungsinfrastruktur behandeln ZUGFeRD-Dokumente schlicht als gewöhnliche PDFs, während systemseitig die XML-Nutzlast selektiv extrahiert und weiterverarbeitet werden kann.\footnote{Vgl. Forum elektronische Rechnung Deutschland (FeRD): \textit{ZUGFeRD 2.3 -- Technische Spezifikation und Implementierungsleitfaden}, BITKOM e.V., Berlin, 2023, \url{https://www.ferd-net.de/download-zugferd}, letzter Zugriff am 07.03.2026; vgl. auch PDF Association: \textit{PDF/A-3 -- Archivable documents with embedded files}, \url{https://www.pdfa.org/pdf-a/}, letzter Zugriff am 07.03.2026}

\subsection{Profile und Konformität}
ZUGFeRD 2.3 setzt die europäische Norm EN 16931 um.\footnote{Vgl. CEN (European Committee for Standardization): \textit{EN 16931-1:2017 -- Electronic invoicing: Semantic data model of the core elements of an electronic invoice}, \url{https://standards.cen.eu/dyn/www/f?p=204:110:0::::FSP_PROJECT:60602}, letzter Zugriff am 07.03.2026} Da die Anforderungen je nach Unternehmensgröße und Branche variieren, definiert ZUGFeRD verschiedene Profile:
...
\texttt{Supply\-Chain\-Trade\-Transaction} $\rightarrow$
\texttt{Included\-Supply\-Chain\-Trade\-Line\-Item} $\rightarrow$
\texttt{Specified\-Line\-Trade\-Agreement} $\rightarrow$
\texttt{Net\-Price\-Product\-Trade\-Price} $\rightarrow$
\texttt{Charge\-Amount}.
Diese hohe Komplexität macht die Implementierung eines Mappers anspruchsvoll, da hunderte von Pfaden korrekt ausgefüllt werden müssen, um Validierungsfehler zu vermeiden.\footnote{Vgl. FeRD e.V.: \textit{ZUGFeRD 2.3 - Das Datenformat für elektronische Rechnungen}, \url{https://www.ferd-net.de/download-zugferd}, letzter Zugriff am 19.12.2025}
...
Interessant ist, dass die Norm EN 16931 zwei völlig unterschiedliche XML-Formate als gleichwertig anerkennt: UBL (ISO/IEC 19845) und UN/CEFACT CII. Das liegt daran, dass verschiedene Länder und Netzwerke unterschiedliche Präferenzen haben. Peppol, das europaweite E-Procurement-Netzwerk, nutzt hauptsächlich UBL, während ZUGFeRD auf CII basiert.\footnote{Vgl. OpenPeppol AISBL: \textit{Peppol BIS Billing 3.0 -- Implementing the European Standard EN 16931 for cross-border invoicing}, \url{https://docs.peppol.eu/poacc/billing/3.0/}, letzter Zugriff am 07.03.2026} Das führt dazu, dass es in Europa kein einheitliches Format gibt.
...
\begin{itemize}
    \item In \textbf{UBL} werden Steuern häufig direkt auf Zeilenebene referenziert (\texttt{Classified\-Tax\-Category}).
...
\begin{itemize}
    ...
    \item Einhaltung der Elementreihenfolge - insbesondere CII toleriert keinerlei Abweichungen.
\end{itemize}
Ein Dokument, das diesen syntaktischen Vertrag verletzt, wird von konformen Empfangssystemen in aller Regel bereits beim Einlesen verworfen, ohne dass eine inhaltliche Prüfung auch nur beginnt.\footnote{Vgl. W3C: \textit{XML Schema Part 1: Structures, Second Edition}, W3C Recommendation, 28 October 2004, \url{https://www.w3.org/TR/xmlschema-1/}, letzter Zugriff am 07.03.2026}

\subsection{Semantische Validierung (Business Rules)}
Nur weil das XML technisch korrekt aufgebaut ist, heißt das noch nicht, dass die Inhalte auch stimmen. Deshalb legt die Norm EN 16931 zusätzlich eine Liste von Geschäftsregeln fest, die sicherstellen sollen, dass die Beträge und Steuern rechnerisch korrekt und widerspruchsfrei sind. Diese Regeln werden meistens mit \textit{Schematron} geprüft.\footnote{Vgl. ISO/IEC 19757-3:2020: \textit{Information technology -- Document Schema Definition Languages (DSDL) -- Part~3: Rule-based validation using Schematron}, International Organization for Standardization; Referenzimplementierung: \url{https://www.schematron.com/}, letzter Zugriff am 07.03.2026}
...
Als OCR-Engine wird \textit{Tesseract} verwendet, ein Open-Source-Projekt das ursprünglich von HP entwickelt wurde und heute von Google weiterentwickelt wird.\footnote{Vgl. Smith, Ray: \textit{An Overview of the Tesseract OCR Engine}, in: Proceedings of the 9th International Conference on Document Analysis and Recognition (ICDAR~2007), IEEE, 2007, S.~629--633, \url{https://doi.org/10.1109/ICDAR.2007.4376991}, letzter Zugriff am 07.03.2026; Quellcode und Dokumentation: \url{https://github.com/tesseract-ocr/tesseract}, letzter Zugriff am 07.03.2026} Ab Version 4.0 basiert Tesseract auf LSTM-Netzen (Long Short-Term Memory), einer Art neuronalem Netz das gut mit sequenziellen Daten wie Text umgehen kann.\footnote{Vgl. Tesseract OCR: \textit{Neural Nets in Tesseract~4.00}, offizielle Entwicklerdokumentation, \url{https://tesseract-ocr.github.io/tessdoc/NeuralNetsInTesseract4.00.html}, letzter Zugriff am 07.03.2026} Dadurch erkennt es Buchstaben deutlich zuverlässiger als ältere regelbasierte Methoden.
...
\begin{enumerate}
    ...
    \item \textbf{Baseline-Fitting}: Erkennen der Grundlinie jeder Textzeile - wichtig damit Buchstaben richtig segmentiert werden können, auch wenn der Scan leicht schief ist.
...
\begin{itemize}
    \item \textbf{Auflösung}: Unter 300 DPI wird die Erkennungsrate deutlich schlechter, weil die feinen Linien einzelner Buchstaben zu wenige Pixel haben um sicher erkannt zu werden.\footnote{Vgl. Tesseract OCR: \textit{Improving the Quality of the Output -- Image Quality and Resolution}, \url{https://tesseract-ocr.github.io/tessdoc/ImproveQuality.html}, letzter Zugriff am 07.03.2026}
...
Moderne LLMs basieren auf der Transformer-Architektur und verwenden Attention-Mechanismen, um Zusammenhänge zwischen verschiedenen Teilen eines Textes zu erkennen.\footnote{Vgl. Vaswani, Ashish et al.: \textit{Attention Is All You Need}, in: Advances in Neural Information Processing Systems~30 (NeurIPS~2017), \url{https://arxiv.org/abs/1706.03762}, letzter Zugriff am 07.03.2026} Wichtig für die Extraktion ist das sogenannte \textit{In-Context Learning}: Das Modell wurde zwar nie speziell auf Rechnungen trainiert, hat aber durch sein allgemeines Training gelernt, dass eine IBAN meistens bei Wörtern wie ,,Bankverbindung`` oder ,,Kontonummer`` steht.\footnote{Vgl. Brown, Tom B. et al.: \textit{Language Models are Few-Shot Learners} (GPT-3), in: Advances in Neural Information Processing Systems~33 (NeurIPS~2020), \url{https://arxiv.org/abs/2005.14165}, letzter Zugriff am 07.03.2026}
Außerdem kann das Modell Synonyme automatisch auflösen: Ob auf der Rechnung ,,Total``, ,,Zahlbetrag``, ,,Rechnungssumme`` oder ,,zu zahlender Betrag`` steht - das Modell erkennt in allen Fällen, dass dasselbe Feld \texttt{TotalAmount} gemeint ist.
...
Das größte Problem bei LLMs ist die Halluzination.\footnote{Vgl. Huang, Lei et al.: \textit{A Survey on Hallucination in Large Language Models: Principles, Taxonomy, Challenges and Open Questions}, arXiv:2311.05232, 2023, \url{https://arxiv.org/abs/2311.05232}, letzter Zugriff am 07.03.2026} LLMs sind so trainiert, dass sie immer eine möglichst sinnvolle Antwort generieren. Wenn auf einer Rechnung zum Beispiel kein Lieferdatum steht, das JSON-Schema aber eines erwartet, dann erfindet das Modell einfach ein Datum - meistens das Rechnungsdatum.
...
Prompt Engineering bedeutet, die Eingabe an das Modell so zu formulieren, dass man die gewünschten Ergebnisse bekommt.\footnote{Vgl. White, Jules et al.: \textit{A Prompt Pattern Catalog to Enhance Prompt Engineering with ChatGPT}, arXiv:2302.11382, 2023, \url{https://arxiv.org/abs/2302.11382}, letzter Zugriff am 07.03.2026} Bei der Datenextraktion geht es dabei nicht um Kreativität, sondern darum, dass das Modell bei gleicher Eingabe immer die gleiche Ausgabe liefert.
...
\begin{itemize}
    ...
    \item \textbf{Rechtliche Unsicherheit}: US-Anbieter fallen unter den CLOUD Act, der amerikanischen Behörden bestimmte Zugriffmöglichkeiten gibt, selbst wenn die Daten auf europäischen Servern gespeichert sind.\footnote{Vgl. U.S. Department of Justice: \textit{Clarifying Lawful Overseas Use of Data Act (CLOUD Act)}, \url{https://www.justice.gov/dag/cloudact}, letzter Zugriff am 07.03.2026} Das EU-US Data Privacy Framework, das solche Datentransfers regelt, ist außerdem politisch unsicher und wurde schon mehrfach vor Gericht angefochten.\footnote{Vgl. Europäische Kommission: \textit{EU-U.S. Data Privacy Framework -- Adequacy Decision}, \url{https://commission.europa.eu/law/law-topic/data-protection/international-dimension-data-protection/eu-us-data-transfers_en}, letzter Zugriff am 07.03.2026}
\end{itemize}

\subsection{Lokale Alternativen}
Eine datenschutzfreundliche Alternative wäre, ein Open-Source-Modell wie Llama\,3\footnote{Vgl. Dubey, Abhimanyu et al.: \textit{The Llama~3 Herd of Models}, Meta AI Research, arXiv:2407.21783, 2024, \url{https://arxiv.org/abs/2407.21783}, letzter Zugriff am 07.03.2026} oder Mistral\footnote{Vgl. Jiang, Albert Q. et al.: \textit{Mistral~7B}, arXiv:2310.06825, Mistral AI, 2023, \url{https://arxiv.org/abs/2310.06825}, letzter Zugriff am 07.03.2026} lokal auf eigener Hardware zu betreiben, zum Beispiel mit dem Tool Ollama.\footnote{Vgl. Ollama: \textit{Get up and running with large language models}, \url{https://ollama.com/}, letzter Zugriff am 07.03.2026} Dann verlassen die Rechnungsdaten nie die eigene Infrastruktur.
