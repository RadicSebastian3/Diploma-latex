...
{
    _logger.LogWarning("Wenig oder kein Text extrahiert, füge Platzhaltertext (Testmodus) als Fallback hinzu");
    result += "\n\n" + GenerateFallbackDemoText();
}
\end{lstlisting}

Die Prüfung schaut auf drei Dinge: Ist der Text komplett leer? Enthält er nur Fehlermeldungen? Oder sind nach dem Herausrechnen der Seiten-Überschriften weniger als 50 Zeichen übrig? Wenn das zutrifft, wird in der aktuellen Projektversion ein kurzer Platzhaltertext angehängt, damit die nachfolgenden Verarbeitungsschritte im Test nicht sofort abbrechen.

Wichtig: In einer Produktivumgebung sollte dieser Platzhalter nicht verwendet werden. Stattdessen ist ein klarer Fehlerstatus im Frontend sinnvoll (damit der Benutzer weiß, dass die Datei unlesbar ist) und anschließend ein OCR-Fallback, falls die Eingabe ein gescanntes PDF war.
...
public class OcrExtractionService : IOcrExtractionService
{
        ...
        IConfiguration configuration)
    {
        ...
        // Prüfe ob tessdata Ordner existiert
    ...
    Stream imageStream, string fileName)
{
    try
    {
        ...
        try
        {
            ...
            // OCR-Konfiguration für bessere Genauigkeit
...
public async Task<Invoice> ProcessInvoiceAsync(Stream pdfStream, string fileName)
{
    ...
    if (fileName.EndsWith(".pdf", StringComparison.OrdinalIgnoreCase))
    {
        ...
        // Prüfe ob PDF-Extraktion erfolgreich war
        if (string.IsNullOrWhiteSpace(extractedText) || extractedText.Length < 100)
        {
            ...
            // Hier könnte OCR-Fallback getriggert werden
        }
    }
    else
    {
...
\begin{lstlisting}[language={[Sharp]C}, caption={OCR-Fallback für gescannte PDFs (Konzept)}, label={lst:ocr_fallback}]
if (string.IsNullOrWhiteSpace(extractedText) || extractedText.Length < 100)
{
