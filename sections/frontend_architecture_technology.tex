...
Für das \textit{SmartBillConverter}-Frontend fiel die Wahl auf Angular 19 aus folgenden Gründen:
...
Das \textit{SmartBillConverter}-Frontend folgt der modularen Angular-Architektur mit Komponenten, Services und Routing:
...
Die \textit{SmartBillConverter}-Anwendung besteht aus drei Hauptkomponenten:
...
\begin{itemize}
    \item \textbf{@Injectable}: Macht den Service für andere Komponenten verfügbar. \texttt{providedIn: 'root'} bedeutet, dass es nur eine Instanz im gesamten Projekt gibt.\footnote{Vgl. Angular Documentation: \textit{Injectable Decorator}, \url{https://angular.io/api/core/Injectable}, abgerufen am 15.01.2025}
...
Hier wird der \texttt{Router}-Service verwendet, um per Code zu einer anderen Seite zu wechseln.\footnote{Vgl. Angular Documentation: \textit{Router}, \url{https://angular.io/api/router/Router}, abgerufen am 15.01.2025} Die Methode \texttt{navigate()} bekommt ein Array mit dem Pfad und den Parametern (z.B. \texttt{['/processing', 123]} wird zu \texttt{/processing/123}).
