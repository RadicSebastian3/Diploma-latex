...
Die Transformation von unstrukturiertem Text in standardkonformes XML ist der technisch anspruchsvollste Teil des \textit{SmartBillConverter}. Dieses Kapitel dokumentiert die Implementierung der Large Language Model (LLM) Integration mit Google Gemini sowie die Generierung von ebInterface 6.1 und ZUGFeRD 2.3 konformen XML-Dokumenten.
...
Die \texttt{LlmResponse} bündelt alle Verarbeitungsergebnisse: das geparste Datenmodell, die generierten XMLs für beide Standards, Validierungsergebnisse und einen Audit-Trail der Verarbeitungsschritte. Dies ermöglicht Debugging und Compliance-Nachweise.
...
Das Modell ist bewusst flach gehalten, um die JSON-Serialisierung für das LLM zu vereinfachen. Die separaten Biller/Recipient-Felder ermöglichen einfaches Mapping auf ebInterface (flache Struktur) und ZUGFeRD (verschachtelte Struktur).

Die kritische \texttt{IsNetto}-Property löst ein fundamentales Problem: Rechnungen können Beträge als Netto+MwSt oder als Brutto (inkl. MwSt) angeben. Das LLM muss diese Semantik verstehen und dem Backend mitteilen, damit die Steuerberechnung korrekt erfolgt.
...
- Gesamtsumme glatt (100,00€) aber Steuer krumm (16,67€) = Gesamtbetrag ist BRUTTO
...
- Positionspreise sind krumme Betraege (11,99€, 8,39€) = oft Brutto
...
Der Prompt ist das Ergebnis von Dutzenden Iterationen. Jede Regel adressiert ein spezifisches Halluzinations-Problem, das in Tests aufgetreten ist. Beispiel: Ohne die Dezimalstellen-Regel interpretierte das LLM "150,00" oft als "15000.00" (fehlende Komma-Konvertierung).
...
Die \texttt{HttpClient}-Injection via Constructor ist Best Practice für Testbarkeit und Connection-Pooling. Der API-Key wird aus Environment Variables geladen (Fail-Fast bei Fehlen).

Die \texttt{generationConfig} ist kritisch:
\begin{itemize}
    \item \texttt{temperature = 0.0}: Maximale Determinismus (keine Kreativität)
    \item \texttt{response\_mime\_type = "application/json"}: Erzwingt JSON-Output
    \item \texttt{response\_schema}: Definiert die exakte JSON-Struktur (Type Safety)
\end{itemize}

Der \texttt{response\_schema}-Parameter nutzt Geminis "Constrained Decoding": Das Modell kann nur Tokens generieren, die dem Schema entsprechen. Dies verhindert syntaktisch invalides JSON.\footnote{Vgl. Google AI: \textit{Gemini API - Structured Output}, \url{https://ai.google.dev/tutorials/structured_output}, letzter Zugriff am 06.01.2026}
...
Die Logik ist mathematisch präzise:
\begin{itemize}
...
\texttt{RoundAmount} nutzt \texttt{MidpointRounding.AwayFromZero} (kaufmännisches Runden): 2.125 wird auf 2.13 gerundet, nicht 2.12. Dies ist steuerrechtlich korrekt.\footnote{Vgl. § 16 UStG: \textit{Bemessungsgrundlage}, Rundung auf zwei Nachkommastellen}
...
\texttt{XmlSerializerNamespaces} verhindert Auto-Generated Namespace-Prefixes (xsi, xsd). \texttt{XmlWriterSettings} konfiguriert Formatierung: \texttt{Indent = true} für lesbare XML, \texttt{OmitXmlDeclaration = false} fügt \texttt{<?xml version="1.0"?>} hinzu.
...
Typische Fehler:
\begin{itemize}
    \item Fehlende Pflichtfelder: "Element 'Invoice' missing required child 'InvoiceNumber'"
    \item Falsches Datumsformat: "Value '01.01.2024' is invalid according to datatype 'date'"
    \item Ungültige Enum-Werte: "Value 'XYZ' is not valid for attribute 'Currency'"
\end{itemize}

\section{ZUGFeRD 2.3 CII-XML-Generierung}
\label{sec:zugferd_generation}

ZUGFeRD ist strukturell komplexer als ebInterface. Die CII-Syntax (Cross Industry Invoice) ist stark verschachtelt.
...
ZUGFeRD nutzt vier XML-Namespaces:
...
Die \texttt{GuidelineSpecifiedDocumentContextParameter.ID} deklariert das Profile. Der Wert \texttt{urn:cen.eu:en16931:2017} signalisiert EN 16931-Konformität. Dies ist für B2G (Business-to-Government) Pflicht.\footnote{Vgl. European Commission: \textit{Directive 2014/55/EU on electronic invoicing}, \url{https://ec.europa.eu/digital-building-blocks/wikis/display/DIGITAL/eInvoicing}, letzter Zugriff am 06.01.2026}
...
Diese Komplexität ist der Preis für UN/CEFACTs "universelles" Modell. Im Projekt wurden Helper-Methoden erstellt, um diese Verschachtelung zu abstrahieren.

\subsection{PDF/A-3 Generierung: Mögliche zukünftige Erweiterung}
ZUGFeRD ist konzeptionell ein hybrides Format: PDF mit eingebettetem XML. Im aktuellen Projektstand wird ausschließlich das XML generiert (siehe \texttt{ZugferdService.GenerateZugferdXmlAsync}). Die vollständige PDF/A-3-Generierung mit eingebettetem XML wurde als mögliche zukünftige Erweiterung evaluiert, aber \textbf{nicht implementiert}.
...
Diese Funktionalität wurde im Projekt nicht implementiert. Der aktuelle \texttt{ZugferdService} liefert ausschließlich XML-Strings zurück. Für Production-Systeme wäre die vollständige PDF/A-3-Generierung mit Templates (z.B. Razor Views zu HTML, dann HTML zu PDF via Playwright oder wkhtmltopdf) eine sinnvolle Erweiterung.\footnote{Vgl. iText Software: \textit{iText 7 - PDF/A-3 and attachments}, \url{https://kb.itextpdf.com/home/it7kb/ebooks/itext-7-jump-start-tutorial-for-java/chapter-7-creating-pdfs-with-pdf-a}, letzter Zugriff am 06.01.2026}
...
\begin{itemize}
    \item Fehlende Pflichtfelder werden als \texttt{XmlSeverityType.Error} gemeldet
    \item Typ-Mismatchs (z.B. String statt Decimal) führen zu Parsing-Errors
    \item Namespaces-Fehler werden durch Schema-Abgleich erkannt
\end{itemize}

Die \texttt{ValidationErrors}-Liste im \texttt{LlmResponse} gibt dem Frontend detailliertes Feedback für Debugging.

\subsection{Schematron-Validierung: Mögliche zukünftige Erweiterung}
Während XSD die syntaktische Korrektheit sicherstellt, würde Schematron die semantische Validierung ermöglichen (Business Rules wie BR-S-08). Diese Funktionalität wurde im Projekt \textbf{nicht implementiert}, aber als potenzielle Erweiterung evaluiert.
